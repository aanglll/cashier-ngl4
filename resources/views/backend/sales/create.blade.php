    ...
    <script>
        ...
        document.addEventListener('click', function(e) {
            if (e.target.classList.contains('select-product')) {
                ...
                const stock = parseInt(e.target.dataset.stock); // Pastikan stock dalam bentuk angka
        ...
        // Cegah input qty melebihi stock
        document.addEventListener('input', function(e) {
            if (e.target.classList.contains('qty')) {
                const maxStock = parseInt(e.target.getAttribute('max')); // Ambil stok maksimum
                let value = parseInt(e.target.value);

                if (value > maxStock) {
                    e.target.value = maxStock; // Jika melebihi, set ke max
                } else if (value < 1 || isNaN(value)) {
                    e.target.value = 1; // Jika kurang dari 1, set ke minimal
                }
            }
        });
